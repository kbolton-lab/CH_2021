...
id = unlist(read.table(id_path,stringsAsFactor=F,sep="\t",quote="",header=T))

cna = read.table(file=cna_path,stringsAsFactor=F,header=T,sep="\t",colClasses="character")
...
cna$sample_id = gsub(".-...-....-...CEL","",cna$SAMPLE)
cna = cna[which(is.element(cna$sample_id,id)),]
...
cna$DENSE = as.numeric(cna$CF)

cna$type = cna$TYPE
cna$type = cna$CHROM
cna$beg_GRCh37 = cna$BEG_GRCh37
cna$end_GRCh37 = cna$END_GRCh37
...
for(i in list_a){
	...
	list_cna_chr_a[i] = paste(c(i,paste(list_cna_a,collapse=",")),collapse=":")
}
...
cna_chr_d = cna_chr_d[sort_d,]
list_d = unique(cna_chr_d$sample_id)
list_cna_chr_d = rep(NA,length(list_d))
names(list_cna_chr_d) = list_d
for(i in list_d){
	cur_cna_chr_d = cna_chr_d[which(cna_chr_d$sample_id==i),]
	...
	list_cna_chr_d[i] = paste(c(i,paste(list_cna_d,collapse=",")),collapse=":")
}
...
cna_chr_u = cna_chr_u[sort_u,]
list_u = unique(cna_chr_u$sample_id)
list_cna_chr_u = rep(NA,length(list_u))
names(list_cna_chr_u) = list_u
for(i in list_u){
	cur_cna_chr_u = cna_chr_u[which(cna_chr_u$sample_id==i),]
	...
	list_cna_chr_u[i] = paste(c(i,paste(list_cna_u,collapse=",")),collapse=":")
}
...
cna_chr_o = cna_chr_o[sort_o,]
list_o = unique(cna_chr_o$sample_id)
list_cna_chr_o = rep(NA,length(list_o))
names(list_cna_chr_o) = list_o
for(i in list_o){
	cur_cna_chr_o = cna_chr_o[which(cna_chr_o$sample_id==i),]
	...
	list_cna_chr_o[i] = paste(c(i,paste(list_cna_o,collapse=",")),collapse=":")
}
...
mut = read.table(mut_path,stringsAsFactor=F,header=T,quote="",sep="\t",comment.char="")
...
tag = paste0("CH_ideogram_TCGA_chr",chr,".pdf")
pdf(file=tag,width=10,height=40)

plot(
NULL,NULL,
xlim=c(-1.2,10.2),ylim=c(-35,5),
...
pdf("ideogram_legend.pdf",width=10,height=40)
plot(
NULL,NULL,
xlim=c(-1.2,10.2),ylim=c(-35,5),
axes=FALSE,xlab="",ylab=""
