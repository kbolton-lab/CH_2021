...
mut_path = ## import mutation data on JGA
cna_path = ## import cna data on JGA
...
## number of subjects with significant combinations
...
a5 = mat_mut[,"TET2"]!=0 & (mat_cna[,"14:q"]==-2)
aa = sum(a1 | a2 | a3 | a4 | a5)

## not accounting for significant combinations
N = length(id)
...
## accounting for significant combinations
N2 = N - aa
m2 = m - aa
c2 = c - aa
its2 = its - aa
print(fisher.test(matrix(c(its2,m2-its2,c2-its2,N2-m2-c2+its2),ncol=2)))
...
cna_label = names(sort(apply(mat_cna!=0,2,sum),decreasing=T))
